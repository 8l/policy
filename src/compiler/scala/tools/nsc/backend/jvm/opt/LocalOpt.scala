...
import org.objectweb.asm.{Opcodes, MethodWriter, ClassWriter}
import org.objectweb.asm.tree.analysis.{Analyzer, BasicValue, BasicInterpreter}
...
class LocalOpt(settings: ScalaSettings) {
  ...
  private def parametersSize(method: MethodNode): Int = {
    ...
    val paramsSize = org.objectweb.asm.Type.getArgumentTypes(method.desc).iterator.map(_.getSize max 1).sum
